...
void main() 
{  
    vec2 readPointCood = mix(TexCoord, TexCoord2, gl_PointCoord);
    vec4 texColor_0 = texture2D(sampler2d_0, readPointCood) * DestinationColor;
...
void main() {
  ...
  for (float i = 1.0f; i <= numBlurPixelsPerSide; i++) { 
    avgValue += texture2D(blurSampler, readPointCood.xy - i * blurSize * 
